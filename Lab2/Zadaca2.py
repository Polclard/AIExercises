...
class Football(Problem):
    ...
    def check_valid(state, opponents):
        ...
        return man_pos[0] >= 0 and man_pos[0] < 8 and \
                ...
                ball_pos != man_pos and checkTheOpponents and \
    ...
    def successor(self, state):
        ...
        #region MoveUp        
        newManPos = (manX, manY+1)
        if newManPos == ball:
            ball_new = (ballX, ballY+1)
        else:
            ball_new = ball
        newState = (newManPos, ball_new)
        if self.check_valid(newState, self.opponents):
            if ball_new != ball:
                successors["Turni topka gore"] = newState
            else:
                successors["Pomesti coveche gore"] = newState
        #endregion
        ...
        if self.check_valid(newState, self.opponents):
            if ball_new != ball:
                successors["Turni topka dolu"] = newState
            else:
                successors["Pomesti coveche dolu"] = newState
        #endregion
        
        #region MoveRight        
        newManPos = (manX+1, manY)
        if newManPos == ball:
            ball_new = (ballX+1, ballY)
        else:
            ball_new = ball
        newState = (newManPos, ball_new)
        if self.check_valid(newState, self.opponents):
            if ball_new != ball:
                successors["Turni topka desno"] = newState
            else:
                successors["Pomesti coveche desno"] = newState
        #endregion
        
        #region MoveUpRight        
        newManPos = (manX+1, manY+1)
        if newManPos == ball:
            ball_new = (ballX+1, ballY+1)
        else:
            ball_new = ball
        newState = (newManPos, ball_new)
        if self.check_valid(newState, self.opponents):
            if ball_new != ball:
                successors["Turni topka gore-desno"] = newState
            else:
                successors["Pomesti coveche gore-desno"] = newState
        #endregion
        
        #region MoveDownRight       
        newManPos = (manX+1, manY-1)
        if newManPos == ball:
            ball_new = (ballX+1, ballY-1)
        else:
        ...
        if self.check_valid(newState, self.opponents):
            if ball_new != ball:
                successors["Turni topka dolu-desno"] = newState
            else:
                successors["Pomesti coveche dolu-desno"] = newState
        #endregion
    ...
    def h(self, node):
        ballX = node.state[1][0]
        ballY = node.state[1][1]
        
        goalOneX = self.goal[0][0]
        goalOneY = self.goal[0][1]
        
        goalTwoX = self.goal[1][0]
        goalTwoY = self.goal[1][1]
        
        chebyshev_distance = max(self.chebyshevDistance(ballX,ballY,goalOneX,goalOneY), self.chebyshevDistance(ballX,ballY,goalTwoX,goalTwoY))
        euclidean_distance = max(self.euclideanDistance(ballX,ballY,goalOneX,goalOneY), self.euclideanDistance(ballX,ballY,goalTwoX,goalTwoY))
        manhattan_distance = max(self.manhattanDistance(ballX,ballY,goalOneX,goalOneY), self.manhattanDistance(ballX,ballY,goalTwoX,goalTwoY))
        return chebyshev_distance
        # return euclidean_distance
        # return manhattan_distance
        #return max(chebyshev_distance, euclidean_distance, manhattan_distance)
        # return abs(chebyshev_distance - manhattan_distance)
        

'''
"Pomesti coveche gore/dolu/desno/gore-desno/dolu-desno"

"Turni topka gore/dolu/desno/gore-desno/dolu-desno"
'''
...
if __name__ == '__main__':
    ...
    problem = Football((man_pos, ball_pos), opponents, ((7,2),(7,3)))
