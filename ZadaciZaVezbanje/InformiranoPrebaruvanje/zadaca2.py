...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        # print(state)
        if state not in walls and 0 <= state[0] < n and 0 <= state[1] < n:
            return True
        return False
    ...
    def successor(self, state):
        ...
        # ghost, holes, mapLength, goal position

        ghostPosition = state
        ghostX = state[0]
        ghostY = state[1]

        currWalls = self.walls
        mapLength = self.n
        goalPosition = self.goal


        # move Right one position
        newGhostPosition = (ghostX+1, ghostY)
        if self.check_valid(newGhostPosition, currWalls, mapLength):
            successors["Desno 1"] = newGhostPosition
        #move Right two positions
        newGhostPosition = (ghostX+2, ghostY)
        if self.check_valid(newGhostPosition, currWalls, mapLength):
            successors["Desno 2"] = newGhostPosition
        #move Right three positions
        newGhostPosition = (ghostX+3, ghostY)
        if self.check_valid(newGhostPosition, currWalls, mapLength):
            successors["Desno 3"] = newGhostPosition

        #move Up one position
        newGhostPosition = (ghostX, ghostY+1)
        if self.check_valid(newGhostPosition, currWalls, mapLength):
            successors["Gore 1"] = newGhostPosition
        #move Up two positions
        newGhostPosition = (ghostX, ghostY+2)
        if self.check_valid(newGhostPosition, currWalls, mapLength):
            successors["Gore 2"] = newGhostPosition
        #move Up three positions
        newGhostPosition = (ghostX, ghostY+3)
        if self.check_valid(newGhostPosition, currWalls, mapLength):
            successors["Gore 3"] = newGhostPosition


        # print(ghostPosition, currWalls)
...
if __name__ == '__main__':
    '''
        NXN maze in which a ghost moves on a rollers.
        
        Walls presented by black dots that can be jumped over
        
        The ghost needs to come to pacman without bumping into a wall
        or get out of the map.
            
        Allowed moving directions: up, right
        
        With one move the ghost can move for one, two or three positions 
        up or right
