...
if __name__ == '__main__':
    ...
    number_of_neurons = int(input())
    learning_rate = float(input())
    delete_index = int(input())
    specimen = [float(item) for item in input().split(' ')]

    train_set = dataset[:int(len(dataset) * 0.8)]
    validation_set = dataset[int(len(dataset) * 0.8):]
    ...
    classifier = MLPClassifier(hidden_layer_sizes=number_of_neurons,
                               max_iter=20,
                               activation="relu",
    ...
    accuracy_train = float(accuracy_counter_train) / len(train_set)
