...
def accuracy(set1_, set2_, classifier1_, classifier2_, classifier3_):
    accuracy_counter = 0
    for item, real_class in zip(set1_, set2_):
        predicted_class_c0 = classifier1_.predict([item])[0]
        predicted_class_c1 = classifier2_.predict([item])[0]
        predicted_class_c2 = classifier3_.predict([item])[0]
        counter_0 = 0
        counter_1 = 0
        counter_2 = 0
        if predicted_class_c0 == real_class:
            counter_0 += 1
        if predicted_class_c1 == real_class:
            counter_1 += 1
        if predicted_class_c2 == real_class:
            counter_2 += 1
        if counter_0 + counter_1 + counter_2 >= 2:
            accuracy_counter += 1
    return float(accuracy_counter) / len(set1_)
...
if __name__ == '__main__':
    ...
    train_set_naive_bayes = class_0[:int(len(class_0) * x1)] \
                            + class_1[:int(len(class_1) * x1)] \
                            + class_2[:int(len(class_2) * x1)]
    train_x_naive_bayes = [row[:-1] for row in train_set_naive_bayes]
    train_y_naive_bayes = [row[-1] for row in train_set_naive_bayes]
    naive_bayes_classifier.fit(train_x_naive_bayes, train_y_naive_bayes)
    # endregion

    # region Decision Tree Classifier
    decision_tree_classifier = DecisionTreeClassifier(random_state=0)
    train_set_decision_tree = class_0[int(len(class_0) * x1):int(len(class_0) * x2)] \
                              + class_1[int(len(class_1) * x1):int(len(class_1) * x2)] \
                              + class_2[int(len(class_2) * x1):int(len(class_2) * x2)]
    train_x_decision_tree = [row[:-1] for row in train_set_decision_tree]
    train_y_decision_tree = [row[-1] for row in train_set_decision_tree]
    decision_tree_classifier.fit(train_x_decision_tree, train_y_decision_tree)
    # endregion

    # region Random Forest Classifier
    random_forest_set_classifier = RandomForestClassifier(n_estimators=3,
                                                          random_state=0)
    train_set_random_forest = class_0[:int(len(class_0) * x2)] \
                              + class_1[:int(len(class_1) * x2)] \
                              + class_2[:int(len(class_2) * x2)]
    train_x_random_forest_set = [row[:-1] for row in train_set_random_forest]
    train_y_random_forest_set = [row[-1] for row in train_set_random_forest]
    random_forest_set_classifier.fit(train_x_random_forest_set, train_y_random_forest_set)
    # endregion
    ...
    final_accuracy = accuracy(test_x,
                              test_y,
                              naive_bayes_classifier,
                              decision_tree_classifier,
                              random_forest_set_classifier)

    print(f"Tochnost: {final_accuracy}")
