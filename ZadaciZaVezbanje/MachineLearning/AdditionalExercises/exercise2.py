...
if __name__ == '__main__':
    ...
    for row in dataset:
        ...
        for i in range(len(row)):
            if i != least_important_feature_index:
                new_row.append(row[i])
        new_dataset.append(new_row)

    new_test_set = new_dataset[:int(len(new_dataset) * x)]
    new_train_set = new_dataset[int(len(new_dataset) * x):]
    ...
    neural_network_classifier_1 = MLPClassifier(hidden_layer_sizes=15,
                                                activation="relu",
                                                learning_rate_init=0.001,
                                                max_iter=200,
                                                random_state=0)

    neural_network_classifier_2 = MLPClassifier(hidden_layer_sizes=15,
                                                activation="relu",
                                                learning_rate_init=0.001,
                                                max_iter=200,
                                                random_state=0)

    neural_network_classifier_1.fit(standard_scaler.transform(new_train_x), new_train_y)
    neural_network_classifier_2.fit(min_max_scaler.transform(new_train_x), new_train_y)

    accuracy_counter_1 = 0
    accuracy_counter_2 = 0
    for item, real_class in zip(new_test_x, new_test_y):
        predicted_class_1 = neural_network_classifier_1.predict(standard_scaler.transform([item]))
        predicted_class_2 = neural_network_classifier_2.predict(min_max_scaler.transform([item]))
        if predicted_class_1 == real_class:
            accuracy_counter_1 += 1
        if predicted_class_2 == real_class:
            accuracy_counter_2 += 1
    print(f"Tocnost so StandardScaler: {accuracy_counter_1 / len(new_test_set)}")
    print(f"Tocnost so MinMaxScaler: {accuracy_counter_2 / len(new_test_set)}")
