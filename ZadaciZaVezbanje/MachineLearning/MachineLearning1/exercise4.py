...
if __name__ == '__main__':
    ...
    input_remove_column = int(input())
    number_of_decision_trees = int(input())
    input_criterion = input()
    input_record = [float(item) for item in input().split(' ')]

    new_dataset = []
    for row in dataset:
        new_row = []
        for i in range(len(row)):
            if i != input_remove_column:
    ...
    classifier = RandomForestClassifier(n_estimators=number_of_decision_trees,
    ...
    for item, real_class in zip(test_x, test_y):
        predicted_class = classifier.predict([item])
        if predicted_class == real_class:
            accuracy_counter += 1
    accuracy = float(accuracy_counter) / len(test_set)

    new_input_record = []
    for i in range(len(input_record)):
        if i != input_remove_column:
            new_input_record.append(input_record[i])

    print(f"Accuracy: {accuracy}")
    print(f"{classifier.predict([new_input_record])[0]}")
    print(f"{classifier.predict_proba([new_input_record])[0]}")