...
if __name__ == '__main__':
    ...
    new_dataset = []
    for row in dataset:
        new_float_row = [float(item) for item in row]
        new_dataset.append(new_float_row)
    dataset = new_dataset
    ...
    for item, real_class in zip(test_x, test_y):
        predicted_class = gaussian_classifier.predict([item])
        if predicted_class == real_class:
            accuracy_counter += 1

    print(f"{accuracy_counter / len(test_set)}")

    print(f"{int(gaussian_classifier.predict([input_record])[0])}")
    print(f"{gaussian_classifier.predict_proba([input_record])}")
