...
if __name__ == '__main__':
    ...
    number_of_decision_tree = int(input())
    criterion_input = input()
    record = [int(value) for value in input().split(' ')]
    ...
    classificator = RandomForestClassifier(criterion=criterion_input, n_estimators=number_of_decision_tree, random_state=0)
    classificator.fit(train_x, train_y)

    accuracyCounter = 0
    for item, real_class in zip(test_x, test_y):
        predicted_value = classificator.predict([item])
        if predicted_value == real_class:
            accuracyCounter += 1

    accuracy = accuracyCounter / len(test_set)
    ...
    for i in range(len(record)):
        if i != col_index:
            fixed_record.append(record[i])

    input_probability = classificator.predict([fixed_record])
    print(input_probability[0])
    input_probabilities = classificator.predict_proba([fixed_record])
    print(input_probabilities[0])
